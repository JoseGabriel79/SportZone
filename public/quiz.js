...
async function excluirPergunta() {
    ...
    for (i = 0; i < listaDePerguntas.length; i++) {
        perguntasExcluir += i + ". " + listaDePerguntas[i].pergunta + "\n"
    }
    let indiceSelecionado = prompt("Qual pergunta deseja excluir? /Escolha uma opção com seu valor numerico!\n\n" + perguntasExcluir)
    // Verifica se o índice é válido
    const perguntaSelecionada = listaDePerguntas[indiceSelecionado];
    ...
    if (req.ok) {
        ...
        montarPerguntas(); // Atualiza a lista na interface
    } else {
